...
export default class AnnouncementsWebPart extends BaseClientSideWebPart<IAnnouncementsWebPartProps> {
  ...
  private _getListItemsAnnouncements():Promise<IAnnouncementsList>{
    ...
    var query = this.context.pageContext.web.absoluteUrl+"/_api/web/lists/getbytitle('"+this.properties.ListName+"')/items?$select=Title,Description,Effectivedate,Expirydate,Created,ID&$orderby=ID desc&$filter=Effectivedate le datetime'"+dateFormat+"' and Expirydate ge datetime'"+dateFormat+"'";
    console.log(query);
